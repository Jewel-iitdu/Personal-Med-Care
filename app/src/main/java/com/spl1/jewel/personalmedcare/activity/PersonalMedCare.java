...
import com.spl1.jewel.personalmedcare.alarm.Alarm;
import com.spl1.jewel.personalmedcare.alarm.AlarmListAdapter;
import com.spl1.jewel.personalmedcare.R;

//  Toast.makeText(getApplicationContext(), "Delete" + index, Toast.LENGTH_SHORT).show();

public class PersonalMedCare extends Activity
{
  private final String TAG = "PersonalMedCare";
  ...
  public void onCreate(Bundle bundle)
  {
    ...
    Log.i(TAG, "PersonalMedCare.onCreate()");

    mAlarmList = (ListView)findViewById(R.id.alarm_list);

    mAlarmListAdapter = new AlarmListAdapter(this,username);
  ...
  {
    super.onDestroy();
    Log.i(TAG, "PersonalMedCare.onDestroy()");
//    mAlarmListAdapter.save();
  ...
  {
    super.onResume();
    Log.i(TAG, "PersonalMedCare.onResume()");
    mAlarmListAdapter.updateAlarms();
  }

  public void onAddAlarmClick(View view)
  {
    Intent intent = new Intent(getBaseContext(), EditMed.class);

    mCurrentAlarm = new Alarm(this);
    mCurrentAlarm.toIntent(intent);

    PersonalMedCare.this.startActivityForResult(intent, NEW_ALARM_ACTIVITY);
  }
  ...
  private AdapterView.OnItemClickListener mListOnItemClickListener = new AdapterView.OnItemClickListener()
  {
    public void onItemClick(AdapterView<?> parent, View view, int position, long id)
    {
      ...
      mCurrentAlarm.toIntent(intent);
      PersonalMedCare.this.startActivityForResult(intent, EDIT_ALARM_ACTIVITY);
    }
  };
